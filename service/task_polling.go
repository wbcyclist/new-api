...
// 优先级：1. PerCallBilling 任务跳过差额结算
//
//  2. adaptor.AdjustBillingOnComplete 返回正数 → 使用 adaptor 计算的额度
//     (skipped if adaptor is nil — see below)
//  3. taskResult.TotalTokens > 0 (or CompletionTokens > 0 if total is unset)
//     → 按 token 重算
//  4. 都不满足 → 保持预扣额度不变
//
// Exported so callback handlers can settle through the same code path as
// polling and pick up the per-call billing skip and the
// adaptor.AdjustBillingOnComplete tiered_expr settlement.
//
// adaptor may be nil — useful for callers that don't have access to a
// platform-specific adaptor (e.g. callback handlers when init ordering hasn't
// run yet). The PerCallBilling guard and the token-count fallback both still
// apply, so a nil-adaptor caller still gets the correct skip behaviour.
...
// effectiveTokenCount returns the token count to use for the billing fallback.
// Prefers TotalTokens; falls back to CompletionTokens when the upstream only
// reports the completion side (Volc Ark callbacks routinely do this for video
// tasks). Returning 0 means the caller should keep the pre-consumed quota.
func effectiveTokenCount(taskResult *relaycommon.TaskInfo) int {
	if taskResult == nil {
		return 0
	}
